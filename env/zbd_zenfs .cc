...
Zone* ZonedBlockDevice::AllocateZoneWithSameLevelFiles(const std::vector<uint64_t>& fno_list, const InternalKey smallest, const InternalKey largest) {
    ...
    if (fno_list.empty()) {//没有同层文件的情况
      return allocated_zone;
    }


    if (fno_list.size() == 1) {//只有一个同层文件的情况
     sst_zone_mtx_.lock();
     auto zids = sst_to_zone_.find(fno_list[0])->second;//用文件号查找这个文件号对应的zone号
     for (int zid : zids) {
       Zone* z = id_to_zone_.find(zid)->second;//通过zone号找到zone
       if (!z->open_for_write_ && !z->IsFull() && !z->open_for_write_) {
         allocated_zone = z;//没满也不能写，就分配该zone
       }
     }
    ...
    int idx;
    files_mtx_.lock();
    for (idx = 0; idx < (int)fno_list.size(); idx++) {//遍历文件列表
        InternalKey s, l;
        s = files_.find(fno_list[idx])->second->smallest_;//通过文件号遍历zonefile，得到最小，最大键
    ...
    if ( l_idx < 0 ) { // case(1)待写入文件在同层级文件中key最小
     for (uint64_t fno : fno_list) {
      ...
      for (int zid : zids) {
       ...
       if (!z->open_for_write_ && !z->IsFull()) {
         allocated_zone = z;//找到一个能写入的同层文件就写
         break;
       }
      }
      sst_zone_mtx_.unlock();
     ...
     for (auto it = fno_list.rbegin(); it != fno_list.rend(); ++it) {//倒序遍历list
       sst_zone_mtx_.lock();
...
void ZonedBlockDevice::AllFile(const InternalKey& s, const InternalKey& l,std::vector<uint64_t>& fno_list) {//获取全部层级的文件号
   fno_list.clear();
...
Zone* ZonedBlockDevice::AllocateZone(Env::WriteLifeTimeHint file_lifetime,
  ...
  if (sst_to_zone_.empty()) {//如果没有sst在zone里
    if (active_io_zones_.load() < max_nr_active_io_zones_) {
      for (const auto z : io_zones) {//直接找一个空的zone且原本不能写的zone进行分配
  ...
  //AdjacentFileList(smallest, largest, level, fno_list);//fnolist得到的是上下层级中所有有重叠的list
  GetAllOverlappingFiles(smallest, largest,fno_list)//获取所有层中有重叠的文件
      if (!fno_list.empty()) {
   ...
   if (!fno_list.empty()) {  //重叠文件列表不为空
    ...
    for (uint64_t fno : fno_list) {
      ...
      if (z != sst_to_zone_.end()) {
        for (int zone_id : z->second) {
          zone_list.insert(zone_id);  //把所有zone提到zonelist里
        }
      }
    }
    sst_zone_mtx_.unlock();
    ...
    for (const auto z : io_zones) {
      ...
      if (search != zone_list.end()) {
        if (!z->IsFull() && !z->open_for_write_) {
          candidates.push_back(z);  //将所有zone作为候选
        }
      }
    } 
    ...
    if (!candidates.empty()) {
      ...
      for (const auto z : candidates) {
        ...
        for (const auto ext :z->extent_info_) {  //读到每一个_zone extent里的内容
          if (!ext->valid_) {
  ...
  if (!fno_list.empty()) {  //重叠文件列表不为空
    ...
    if (!candidates.empty()) {
      ...
      for (const auto z : candidates) {
        uint64_t inval_data = 0;
        for (const auto ext :
             z->extent_info_) {  //读到每一个_zone extent里的内容
          if (!ext->valid_) {
            inval_data += ext->length_;
          }
        }
